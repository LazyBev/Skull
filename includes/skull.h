...
// Increased from 1024 to avoid truncation warnings
...
void skull_compile(char* src, const char* output_filename) {
    ...
    // Using PATH_MAX_SIZE * 2 to ensure we have enough space for filenames
    ...
    // Safely create filenames with snprintf
    ...
    // Create command buffers large enough to hold commands
    ...
    if (snprintf(ld_cmd, sizeof(ld_cmd), "%s -o %s", obj_filename, executable_name) >= sizeof(ld_cmd)) {
        fprintf(stderr, "Error: Linking command too long\n");
        free(s);
        exit(1);
    }
