...
void print_usage(const char* program_name) {
    ...
    printf("  -c, --compile      Only compiles and doesnt run\n");
    printf("  -c, --run          Only runs\n");
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if ((strcmp(argv[i], "-c") == 0 || strcmp(argv[i], "--compile") == 0) && !run_only) {
            compile_only = true;
        } else if ((strcmp(argv[i], "-r") == 0 || strcmp(argv[i], "--run") == 0) && !compile_only) {
            run_only = true;
    ...
    skull_compile_file(input_filename, output_filename, run_only, compile_only);
    return 0;
}