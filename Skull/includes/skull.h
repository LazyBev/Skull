...
void skull_compile(char* src, const char* output_filename, bool run_only, bool compile_only) {
    if (!src) {
    ...
    if (snprintf(nasm_cmd, sizeof(nasm_cmd), "-f elf64 %s -o %s", asm_filename, obj_filename) >= sizeof(nasm_cmd)) {
    ...
    if ((run_only && !compile_only) || (!run_only && !compile_only)) {
        // Link the object file to create the executable using ld
        char ld_cmd[PATH_MAX_SIZE * 2];
        if (snprintf(ld_cmd, sizeof(ld_cmd), "-e _start -o %s %s", executable_name, obj_filename) >= sizeof(ld_cmd)) {
        ...
        char run_cmd[PATH_MAX_SIZE * 2];
        if (snprintf(run_cmd, sizeof(run_cmd), "./%s", executable_name) >= sizeof(run_cmd)) {
            fprintf(stderr, "Error: run command too long\n");
            free(s);
            free(root);
            free(parser);
            free(lexer);
        ...
        char* run_output = sh("", run_cmd);
        if (!run_output || strlen(run_output) > 0) {
            fprintf(stderr, "Error: Couldnt run '%s' executable\n", executable_name);
            free(run_output);
            free(s);
            free(root);
            free(parser);
    ...
    // Clean up intermediate files unless keep_intermediate_files is set
...
void skull_compile_file(const char* filename, const char* output_filename, bool run_only, bool compile_only) {
    if (!filename) {
    ...
    skull_compile(src, output_filename, run_only, compile_only);
    free(src);
}
