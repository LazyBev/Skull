...
char* asm_f_assignment(ast_t* ast) {
    ...
    if (ast->value && ast->value->type == AST_FUNCTION) {
        ...
        // Add function epilogue if not present in the function body
        if (strstr(s, "    leave") == NULL && strstr(s, "    ret") == NULL) {
            const char* epilogue = "    leave\n"
                                  "    ret\n\n";
            s = realloc(s, (strlen(s) + strlen(epilogue) + 1) * sizeof(char));
            strcat(s, epilogue);
        }
    }
    return s;
